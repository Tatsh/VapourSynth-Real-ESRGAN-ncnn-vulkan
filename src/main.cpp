...
static void process(const VSFrame *src,
                    VSFrame *dst,
                    const FilterData *const VS_RESTRICT d,
...
static const VSFrame *VS_CC filterGetFrame(int n,
    ...
    if (activationReason == arInitial) {
        ...
        VSFrame *dst =
            vsapi->newVideoFrame(&d->vi->format, d->target_width, d->target_height, src, core);
...
static void VS_CC
filterCreate(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
    ...
    try {
        ...
        const std::string pluginPath{
            vsapi->getPluginPath(vsapi->getPluginByID("com.vapoursynth.realesrgan", core))};
    ...
    */
        ...
        gpuThread =
            std::min(gpuThread, vsh::int64ToIntS(ncnn::get_gpu_info(gpuId).compute_queue_count()));
...
VapourSynthPluginInit2(VSPlugin *plugin, const VSPLUGINAPI *pluginapi) {
    pluginapi->configPlugin("com.vapoursynth.realesrgan",
                            "esrgan",
                            "RealESRGAN ncnn Vulkan plugin",
                            VS_MAKE_VERSION(0, 1),
